...
try:
    ...
    for item in items:
        ...
        if type == "header":
            ...
            offset_to_struct = "offsetof(struct SyobonAnimsObj, " + name + ")"
            offset_to_end = "offsetof(struct SyobonAnimsObj, " + values + ") + sizeof(gSyobonAnims." + values + ")"
    ...
    for item in items:
        ...
        if type == "header":
            ...
            structobj.append("{" + ", ".join([
                ...
                str(v5),
                str(indices_len),
                "(const s16 *)(offsetof(struct SyobonAnimsObj, " + values + ") - " + offset_to_struct + ")",
                "(const u16 *)(offsetof(struct SyobonAnimsObj, " + indices + ") - " + offset_to_struct + ")",
    ...
    print("const struct SyobonAnimsObj {")
    for s in structdef:
        print(s)
    print("} gSyobonAnims = {")
    for s in structobj:
    ...
    note = "NOTE! The syobon animation C files are not processed by a normal C compiler, but by the script in tools/syobon_anims_converter.py. The format is much more strict than normal C, so please follow the syntax of existing files.\n"
    if e is SyntaxError:
        e.msg = note + e.msg
    else:
